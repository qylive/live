...
栖苑太困辽的个人歌单展示网站，支持歌单浏览、搜索筛选、随机点歌、付费标记，以及通过 Excel 上传或粘贴 JSON 更新歌单数据。
...
- 筛选按钮：≥1700px 一行排列；<1700px 切换为三行分组居中（1025-1699px 保持 PC 布局仅筛选栏换行，≤1024px 为移动端整体布局）
- 「前往路灯」按钮位于页头右上角（随页面滚动，不遮挡内容），移动端定位于屏幕右上角
...
### 欢迎动画
- 进入页面时全屏暗色遮罩，居中显示「苑小苑」表情包，弹性弹入后持续心跳脉动
- 下方渐变显示「欢迎来到苑小苑的歌单」，周围带 CSS 爱心与光点装饰
- 约 1.8 秒后整体淡出消失并移除遮罩，不遮挡页面操作
- 尊重系统「减少动态效果」设置

### 歌单管理（upload.html）
- 两种导入方式二选一（switch 切换）：Excel 上传 或 直接粘贴 JSON 歌单数据
- Excel 支持中文分号「；」和英文分号「;」两种曲风分隔方式，第六列可选风格列有值时优先作为 style
- 粘贴 JSON 支持完整歌曲对象数组，自动校验并规范化字段（缺失字段补空、sc 统一为字符串）
- 付费（SC）标记智能合并：导入数据中 SC 为空时，自动沿用线上已有付费标记，避免覆盖丢失
...
| F | 风格 | 可选；有值时优先直接作为风格（style），忽略 C 列分号后的风格 |

> 表头格式不可更改，由上传方使用固定模板；F 列为可选扩展列，可留空。

## 粘贴 JSON 导入

在 `upload.html` 中切换至「粘贴JSON」模式，粘贴歌曲对象数组即可，格式与 `songs.json` 一致（见上方示例）。支持直接从线上 songs.json 复制内容修改后回贴更新。
